(function ($) {
    ...
    $('#search .search-form').on('submit', function () {
        return true;
    });
    ...
    function applyRecipeFilter() {
        ...
        let keyword = $('.recipe-search').val().toLowerCase().trim();
        let selectedCategories = [];
        let selectedCuisines = [];

        $('.advance-search-form input[type="checkbox"]:checked').each(function () {
            let type = $(this).data('type');
            let value = $(this).next('label').text().trim().toLowerCase();
            if (type === 'category') {
        ...
        $('.product-box-layout1').each(function () {

            let $item = $(this);

            let title = $item.find('.item-title').text().toLowerCase();
            let category = $item.find('.sub-title').text().toLowerCase();
            let description = $item.find('p').text().toLowerCase();

            let fullText = title + ' ' + category + ' ' + description;
            ...
            // ===== FINAL =====
            if (textMatch && categoryMatch && cuisineMatch) {
                $item.closest('.col-lg-4').show();
            } else {
                $item.closest('.col-lg-4').hide();
            }
        });
    }

        $('.recipe-search').on('keyup', applyRecipeFilter);

        $('.advance-search-form input[type="checkbox"]').on('change', applyRecipeFilter);
